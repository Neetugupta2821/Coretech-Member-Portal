...
const login = async () => {
    try {
        const credentials = {
            email: email.value, // Use email as username
            password: password.value
        };

        const response = await axios.post('http://192.168.1.83:8000/api/child/login/', credentials);
...
const login = async () => {
  try {
    ...
    const response = await store.dispatch('loginUser', credentials);
    ...
    if (response.data.status === 'success') {
      ...
      store.commit('addToast', { title: 'Login Failed', message: 'Invalid credentials', severity: 'error' }); // Example using toast
    }
  } catch (error) {
    console.error('Login error:', error);
    store.commit('addToast', { title: 'Login Error', message: 'An error occurred during login', severity: 'error' }); // Example using toast
  }
};
</script>
      ...
      <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
        <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
          ...
          <div>
            ...
            <Button label="Sign In" class="w-full" @click="login"></Button> <--- Call login function on button click
          </div>
        </div>
      </div>
    </div>
